...
export default function useCreateIpAsset(
  createReq?: ModifiedCreateIPAssetRequest
) {
  ...
  const execute = useCallback(
    async (
      executeCreateReq?: CreateIpAssetRequest | ModifiedCreateIPAssetRequest
    ) => {
      ...
      try {
        const formattedReq = {
          ..._createReq!,
          typeIndex:
            typeof _createReq!.typeIndex === 'string'
              ? parseInt(_createReq!.typeIndex)
              : _createReq!.typeIndex,
        };

        const createRes: CreateIpAssetResponse = await (
          client as Client
        ).ipAsset.create(formattedReq);
        setIsIdle(false);
